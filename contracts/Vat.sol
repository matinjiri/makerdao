...
contract Vat {
    ...
    function _add(uint x, int y) internal pure returns (uint z) {
        z = x + uint(y);
        require(y >= 0 || z <= x, "Math/add-overflow-neg");
        require(y <= 0 || z >= x, "Math/add-overflow-pos");
    }
    function _sub(uint x, int y) internal pure returns (uint z) {
        z = x - uint(y);
        require(y <= 0 || z <= x, "Math/sub-overflow-pos");
        require(y >= 0 || z >= x, "Math/sub-overflow-neg");
    }
    function _mul(uint x, int y) internal pure returns (int z) {
        z = int(x) * y;
        require(int(x) >= 0, "Math/mul-invalid-x-neg");
        require(y == 0 || z / y == int(x), "Math/mul-overflow");
    }
